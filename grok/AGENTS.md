...
You have access to a large personal skill library in `~/.grok/skills/`, mostly symlinked from `~/.agents/skills/`. Grok also discovers skills from `./.grok/skills/`, `<repo_root>/.grok/skills/`, and `~/.claude/skills/`; same-name skills are deduplicated with the repo-scoped copy winning. Match the task against your native skill mechanism first and read the chosen skill's `SKILL.md` before acting. If native matching is unclear, query the vault instead of guessing:
